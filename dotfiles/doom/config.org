...
* My /n/th literal emacs configuration     :TOC_3:
  ...
  - [[#packages][Packages]]
    ...
    - [[#dim-other-windows][Dim other windows]]
    - [[#fish-shell-mode][Fish shell mode]]
  - [[#look-and-feel][Look and feel]]
    ...
    - [[#dim-other-windows-1][Dim other windows]]
...
#+begin_src emacs-lisp :tangle yes
(cond
 ((string= (system-name) "Nightcrawler.local")
...
I used to set this to =t= for boring old line numbers. I'm taking relative for a bit of a spin for a while to see how it feels. (Disabled now, as it turns out I prefer real numbers.)
...
Disabled. Turns out I /don't/ always want this.
#+begin_src emacs-lisp :tangle no
...
Any language modes that are not already covered by Doom are installed here.
#+begin_src emacs-lisp
...
[[https://github.com/mina86/auto-dim-other-buffers.el][Github project for dim-other-windows]]
#+BEGIN_SRC emacs-lisp
...
:END:
I am enjoying Graphik as the variablch font. Some other options that I may try in the future are.
- Avenir Next
- Founders Grotesk
- Graphik
...
*** Ligatures
I've grown to appreciate the use of ligatures to replace certain elements.
#+begin_src emacs-lisp
(setq +ligatures-in-modes t)
(setq +ligatures-extras-in-modes t)
...
(setq doom-fallback-buffer-name "► Doom"
      fancy-splash-image "~/.doom.d/misc/splash-images/LaunchDarkly_Icon_on_Dark_RGB.png"
...
Currently disabled. For some reason it stopped working with the latest Doom (as at 2020-11-05) so I'm giving Golden Ratio a try out.
...
Automatically resize the active wndow to match golden ratio rules.
#+begin_src emacs-lisp :tangle no
(golden-ratio-mode 1)
...
Disabled org-superstar - so many CPU cycles...
#+begin_src emacs-lisp :tangle no
(after! org-superstar
...
I want to disable ~spell-fu~ in certain modes, and ignore certain sections in others.
#+begin_src emacs-lisp :tangle yes
...
I want to use an index file.
#+begin_src emacs-lisp :tangle yes
  (setq org-roam-index-file "index.org")
#+end_src

In addition to the ~#+ROAM_TAGS~ header, I may decide to organise my roam notes into sub-directories. If I do, then each directory will match a tag hierarchy which can be used in addition to ~#+ROAM_TAGS~ to define tags for the files that they contain. In ~org-roam-tag-sources,~ ~prop~ says to use ~#+ROAM_TAGS~, and ~all-directories~ will take tags from the directory hierarchy below ~org-roam-directory~.
#+begin_src emacs-lisp :tangle yes
